...
class account_class extends AWS_MODEL
{
    ...
    {
        if($user_info=$this->fetch_row('users',"mobile='" . $this->quote($mobile) . "'"))
        {
            if($password){
    ...
    public function get_users_list($where, $limit = 10, $attrib = false, $exclude_self = true, $orderby = 'uid DESC',$attrib_order=null,$attrib_where=null)
    {
        if ($where)
        {
            $where = '(' . $where . ') AND forbidden = 0 AND is_del = 0 AND group_id <> 3';
        }
        else
        {
            $where = 'forbidden = 0 AND is_del = 0 AND group_id <> 3';
        }
    ...
    public function get_user_recommend_v2($uid, $limit = 10)
    {
        ...
        if ($users_list = $this->get_users_list('uid IN(' . implode($friend_uids, ',') . ') AND forbidden = 0 AND is_del=0 AND uid NOT IN (' . implode($follow_uids, ',') . ')', $limit, true, true))
        {
    ...
    public function associate_remote_avatar($uid, $headimgurl)
    {
        ...
        if (!$avatar_stream = file_exists($headimgurl))
        {
            return false;
        }
        ...
        if (!$this->model('integral')->fetch_log(intval($uid), 'UPLOAD_AVATAR'))
        {
            $this->model('integral')->process(intval($uid), 'UPLOAD_AVATAR', round((get_setting('integral_system_config_profile') * 0.2)), '上传头像');
        }
