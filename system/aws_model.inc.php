...
class AWS_MODEL
{
     ...
     * @throws \Zend_Exception
     */
    public function fetch_column($table, $where = null, $order = null, $column=array(),$limit=null,$offset = 0)
    {
        $this->slave();
        $select = $this->select();
        $select->from($this->get_table($table), $column ? $column : '*');
        if ($where)
        {
            $select->where($where);
        }
     ...
     * @throws \Zend_Exception
     */
	public function sum($table, $column, $where = '',$type=2)
	{
		$this->slave();
