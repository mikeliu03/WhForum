...
class posts_class extends AWS_MODEL
{
	...
	public function set_posts_index($post_id, $post_type, $data = null)
	{
		...
		else
		{
			...
			return AWS_APP::model()->insert('posts_index', $data);
		}
	}
	...
	public function get_posts_list($post_type, $page = 1, $per_page = 10, $sort = null, $topic_ids = null, $category_id = null, $answer_count = null, $day = 30, $is_recommend = false)
	{
		...
		switch ($sort)
		{
			...
			case 'new' :
				$order_key = 'set_top_time DESC,update_time DESC';
	...
	public function process_explore_list_data($posts_index)
	{
		...
		if ($article_ids)
		{
            ...
            {
                $article_infos[$k]['img_list'] = get_content_img($v['message'],false);
			...
			switch ($data['post_type'])
			{
				case 'question':
					...
					if($explore_list_data[$key]['answer_info']){
                        $explore_list_data[$key]['img_list'] = get_content_img($last_answers[$data['post_id']]['answer_content'],false);
                    }else{
                        $explore_list_data[$key]['img_list'] = get_content_img($question_infos[$data['post_id']]['question_detail'],false);
                    }
    ...
    public function set_top($question_id)
    {
        ...
        //问题被置顶 体力值变化
		if (!$this->model('integral')->fetch_log($question_info['published_uid'], 'QUESTION_TOP'))
 		{
 			$this->model('integral')->process($question_info['published_uid'], 'QUESTION_TOP', get_setting('question_top'), '问题被置顶 #' . $question_id, $question_id);
 		}
    }
    ...
    /*设置文章置顶*/
    public function set_top_article($article_id)
    {
        ...
        $this->model('posts')->set_posts_index($article_id, 'set_top_article');

        //问题被置顶 体力值变化
		if (!$this->model('integral')->fetch_log($article_info['uid'], 'ARTICLE_TOP'))
 		{
 			$this->model('integral')->process($article_info['uid'], 'ARTICLE_TOP', get_setting('question_top'), '文章被置顶 #' . $article_id, $article_id);
 		}
    }

    /*取消文章置顶*/
    public function unset_top_article($article_id)
    {
        $this->update('article', array(
            'set_top' => 0,
            'set_top_time' => 0
        ...
        $this->model('posts')->set_posts_index($article_id, 'unset_top_article');
    }
