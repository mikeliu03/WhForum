...
class ajax extends AWS_ADMIN_CONTROLLER
{
    ...
    public function login_process_action()
    {
        ...
        if ($user_info['uid'])
        {
            ...
            //记录日志
            action_log('admin_login',$user_info['uid'], $record_id = null,$remark='管理员登陆');
    ...
    public function save_settings_action()
    {
        ...
        if($_FILES['site_logo_url']=='' && get_setting('site_logo_url'))
        {
            unset($_POST['site_logo_url']);
        }else{
            if ($_FILES['site_logo_url']['name'])
            {
                AWS_APP::upload()->initialize(array(
                    'allowed_types' => 'jpg,jpeg,png,gif',
                    'upload_path' => get_setting('upload_dir') . '/logo/',
                    'is_image' => TRUE,
                    'max_size' => get_setting('upload_avatar_size_limit'),
                    'encrypt_name' => FALSE,
                    'file_name' =>'logo'
                ))->do_upload('site_logo_url');
                ...
                $_POST['site_logo_url'] = get_setting('upload_url') . '/logo/' . $upload_data['file_name'];
            }
        }

        if($_FILES['login_logo_url']=='' && get_setting('login_logo_url'))
        {
            unset($_POST['login_logo_url']);
        }else{
            if ($_FILES['login_logo_url']['name'])
            {
                AWS_APP::upload()->initialize(array(
                    'allowed_types' => 'jpg,jpeg,png,gif',
                    'upload_path' => get_setting('upload_dir') . '/logo/',
                    ...
                    'max_size' => get_setting('upload_avatar_size_limit'),
                    'encrypt_name' => FALSE,
                    'file_name' =>'login_logo'
                ))->do_upload('login_logo_url');
                if (AWS_APP::upload()->get_error())
                {
                    switch (AWS_APP::upload()->get_error())
                    {
                ...
                $_POST['login_logo_url'] = get_setting('upload_url') . '/logo/' . $upload_data['file_name'];
            }
        }
        ...
        //设置系统配置
        $debug = $_POST['debug_enable']=='Y' ?  1 : 0;

        AWS_APP::config()->set('system',['debug'=>intval($debug)]);

        $cache_type = $_POST['cache_type'];

        $cache_host = $_POST['cache_host'] ? $_POST['cache_host'] : '127.0.0.1';

        $cache_host_port = $_POST['cache_host_port'];

        if($cache_type =='redis' && !extension_loaded('redis'))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('您还未开启Redis拓展')));
        }

        if($cache_type =='memcached' && !extension_loaded('memcached'))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('您还未开启Memcached拓展')));
        }

        switch ($cache_type)
        {
            case 'Redis':
                AWS_APP::config()->set('cache',[
                    'Memcached'=>array(
                        'host'=>'127.0.0.1',
                        'port'=>11211,
                        'persistent' => true,
                    ...
                    'Redis'=>array(
                        'host'=>$cache_host,
                        'port'=>$cache_host_port ? $cache_host_port : 6379,
            ...
            case 'Memcached':
                AWS_APP::config()->set('cache',[
                    'Memcached'=>array(
                        'host'=>$cache_host,
                        'port'=>$cache_host_port ? $cache_host_port : 11211,
                        'persistent' => true,
                        'timeout' => 60,
                        'compression' => false,
                        'compatibility' => false
        ...
        switch ($_POST['type'])
        {
                ...
                switch ($_POST['batch_type'])
                {
                    case 'approval':
                        ...
                        foreach ($_POST['approval_ids'] AS $approval_id)
                        {
                            ...
                            action_log($_POST['type'].'_'.$_POST['batch_type'] . '_publish',$this->user_id, $approval_id,$_POST['type'].'审核通过记录');
                        }
                    ...
                    case 'decline':
                        foreach ($_POST['approval_ids'] AS $approval_id)
                        {
                            ...
                            //记录日志
                            action_log($_POST['type'].'_'.$_POST['batch_type'] . '_publish',$this->user_id, $approval_id,$_POST['type'].'拒绝审核记录');
                        }
            ...
            default:
                ...
                foreach ($_POST['approval_ids'] AS $approval_id)
                {
                    ...
                    //记录日志
                    action_log($_POST['type'].'_'.$_POST['batch_type'] . '_publish',$this->user_id, $approval_id,$_POST['type'].($_POST['batch_type']=='approval' ? '审核通过记录' : '拒绝审核记录'));
                }
        ...
        switch ($_POST['action'])
        {
            case 'del':
                foreach ($_POST['article_ids'] AS $article_id)
                {
                    ...
                    action_log('article_del',$this->user_id, $article_id,$remark='文章删除');
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
            case 'recover':
                foreach ($_POST['article_ids'] AS $article_id)
                {
                    $this->model('article')->recover_article($article_id);
                    action_log('article_recover',$this->user_id, $article_id,$remark='文章恢复');
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
            case 'clear':
                foreach ($_POST['article_ids'] AS $article_id)
                {
                    ...
                    action_log('article_clear',$this->user_id, $article_id,$remark='清空文章回收站');
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
        }
    ...
    public function save_category_sort_action()
    {
        ...
        if (is_array($_POST['category']))
        {
            foreach ($_POST['category'] as $key => $val)
            {
                ...
                action_log('save_category_sort',$this->user_id, $key,$remark='分类排序');
            }
        }
    ...
    public function save_category_action()
    {
        ...
        else
        {
            ...
            action_log('save_category',$this->user_id, $category_id,$remark='新增分类');
        }
        ...
        action_log('update_category',$this->user_id, $category_id,$remark='更新分类');
    ...
    public function remove_category_action()
    {
        ...
        action_log('delete_category',$this->user_id, $_POST['category_id'],$remark='删除分类');
        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }
    ...
    public function remove_nav_menu_action()
    {
        ...
        action_log('remove_nav_menu',$this->user_id, $_POST['id'],$remark='删除菜单导航');
        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }
    ...
    public function nav_menu_upload_action()
    {
        ...
        action_log('update_nav_menu_icon',$this->user_id, $_GET['id'],$remark='更新菜单导航图标');
    ...
    public function add_page_action()
    {
        ...
        $page_id = $this->model('page')->add_page($_POST['title'], $_POST['keywords'], $_POST['description'], $_POST['contents'], $_POST['url_token']);
        //日志记录
        action_log('add_page',$this->user_id, $page_id,$remark='新增单页');
        H::ajax_json_output(AWS_APP::RSM(array(
    ...
    public function remove_page_action()
    {
        ...
        //日志记录
        action_log('remove_page',$this->user_id, $_POST['id'],$remark='删除单页');
    ...
    public function edit_page_action()
    {
        ...
        //日志记录
        action_log('update_page',$this->user_id, $_POST['page_id'],$remark='编辑单页');
        H::ajax_json_output(AWS_APP::RSM(array(
            'url' => get_js_url('/admin/page/')
        ), 1, null));
    }
        ...
        switch ($_POST['action'])
        {
            case 'remove':
                foreach ($_POST['question_ids'] AS $question_id)
                {
                    ...
                    //日志记录
                    action_log('remove_question',$this->user_id, $question_id,$remark='逻辑删除问题');
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
            case 'recover':
                foreach ($_POST['question_ids'] AS $question_id)
                {
                    ...
                    //日志记录
                    action_log('recover_question',$this->user_id, $question_id,$remark='恢复删除问题');
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
            case 'clear':
                foreach ($_POST['question_ids'] AS $question_id)
                {
                    ...
                    //日志记录
                    action_log('clear_question',$this->user_id, $question_id,$remark='清空问题回收站');
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
        }
    ...
    public function report_manage_action()
    {
        ...
        if ($_POST['action_type'] == 'delete')
        {
            foreach ($_POST['report_ids'] as $val)
            {
                ...
                //日志记录
                action_log('delete_question_report',$this->user_id, $val,$remark='删除问题举报');
            }
        }
        else if ($_POST['action_type'] == 'handle')
        {
            foreach ($_POST['report_ids'] as $val)
            {
                ...
                //日志记录
                action_log('update_question_report',$this->user_id, $val,$remark='处理问题举报');
            }
        }
    ...
    public function save_user_action()
    {
        if ($_POST['uid'])
        {
            ...
            //日志记录
            action_log('update_user_info',$this->user_id, $user_info['uid'],$remark='用户资料更新');
        ...
        else
        {
            ...
            //日志记录
            action_log('register_user',$this->user_id, $uid,$remark='后台添加用户');
            H::ajax_json_output(AWS_APP::RSM(array(
    ...
    public function forbidden_user_action()
    {
        ...
        //日志记录
        action_log('forbidden_user',$this->user_id, $_POST['uid'],$remark='后台封禁用户');
        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }
    ...
    public function integral_process_action()
    {
        ...
        //日志记录
        action_log('update_user_integral',$this->user_id, $_POST['uid'],$remark='后台操作用户积分'.$_POST['integral']);
        H::ajax_json_output(AWS_APP::RSM(array(
        ...
        switch ($_POST['batch_type'])
        {
            case 'approval':
                foreach ($_POST['approval_uids'] AS $approval_uid)
                {
                    ...
                    //日志记录
                    action_log('approval_user_register',$this->user_id, $approval_uid,$remark='通过用户注册审核');
                }
            break;
            ...
            case 'decline':
                foreach ($_POST['approval_uids'] AS $approval_uid)
                {
                    if ($user_info = $this->model('account')->get_user_info_by_uid($approval_uid))
                    {
                        ...
                        //日志记录
                        action_log('decline_user_register',$this->user_id, $approval_uid,$remark='拒绝用户注册审核');
                    }
                }
            break;
        }
        ...
        switch ($_POST['batch_type'])
        {
            ...
            case 'decline':
                ...
                foreach ($_POST['approval_ids'] AS $approval_id)
                {
                    ...
                    //日志记录
                    action_log($_POST['batch_type'] . '_verify',$this->user_id, $approval_id,$remark='认证审核');
                }
            break;
        }
        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    ...
    public function remove_user_action()
    {
        ...
        //日志记录
        action_log('remove_user',$this->user_id, $_POST['uid'],$remark='删除用户');
        H::ajax_json_output(AWS_APP::RSM(array(
            'url' => get_js_url('/admin/user/list/')
        ), 1, null));
    }
    ...
    public function remove_users_action()
    {
        ...
        foreach ($_POST['uids'] AS $uid)
        {
            ...
            //日志记录
            action_log('remove_user',$this->user_id, $uid,$remark='删除用户');
        }
        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }
    ...
    public function topic_statistic_action()
    {
	    ...
	    H::ajax_json_output($topic_statistic);
    }
    ...
    public function remove_recover_action()
    {
        ...
        foreach ($_POST['uids'] AS $uid) {
            ...
            //日志记录
            action_log('recover_user',$this->user_id, $uid,$remark='恢复用户');
        }
        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }
        ...
        switch ($_POST['action'])
        {
            case 'del':
                foreach ($_POST['ids'] AS $comment_id)
                {
                    ...
                    //日志记录
                    action_log('remove_article_comment',$this->user_id, $comment_id,$remark='删除文章评论');
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
                break;
            case 'recover':
                foreach ($_POST['ids'] AS $comment_id)
                {
                    ...
                    //日志记录
                    action_log('recover_article_comment',$this->user_id, $comment_id,$remark='恢复文章评论');
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
                break;
            case 'clear':
                foreach ($_POST['ids'] AS $comment_id)
                {
                    $this->model('article')->remove_comment_phy($comment_id);
                    //日志记录
                    action_log('clear_article_comment',$this->user_id, $comment_id,$remark='清空回收站文章评论');
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
        }
