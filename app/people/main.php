...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		...
		if (!$user)
		{
			H::redirect_msg('用户不存在或已被删除');
		}
		if($this->user_id and get_setting('payment')=='Y')
		{
			$balance = $this->model('account')->get_banlce($this->user_id);
			TPL::assign('balance', $balance['balance']);
			TPL::assign('haspass', $balance['deal_pwd']?1:0);
		}
		...
		TPL::assign('user_actions_questions', $this->model('actions')->get_user_actions($user['uid'], get_setting('contents_per_page'), ACTION_LOG::ADD_QUESTION, $this->user_id));
		TPL::assign('user_actions_questions_count', $this->model('actions')->get_user_actions_count($user['uid'],ACTION_LOG::ADD_QUESTION, $this->user_id));
		...
		TPL::assign('user_actions_answers_count',  $this->model('actions')->get_user_actions_count($user['uid'], ACTION_LOG::ANSWER_QUESTION, $this->user_id));
		TPL::assign('user_actions_articles_count',  $this->model('actions')->get_user_actions_count($user['uid'], ACTION_LOG::ADD_ARTICLE, $this->user_id));
		TPL::assign('user_actions', $this->model('actions')->get_user_actions($user['uid'], get_setting('contents_per_page'), implode(',', array(
