...
*/
    ...
    public function save_template_msg_action()
    {
        $data=array(
            'template_id' => trim($_POST['template_id']),
            'template_type' => trim($_POST['template_type']),
            'template_data' => trim(str_replace(' ', '',$_POST['template_data'])),
        );
        if(intval($_POST['id']))
        {
            if($this->model('weixin')->update('weixin_template',$data,'id = '.intval($_POST['id'])))
            {
                H::ajax_json_output(AWS_APP::RSM(null, -1, '更新成功'));
            }
        }else{
            if($this->model('weixin')->insert('weixin_template',$data))
            {
