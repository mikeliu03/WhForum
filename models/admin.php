...
class admin_class extends AWS_MODEL
{
    ...
    {
        try {
            $last_version = AWS_APP::model('cloud')->check_last_version();
        } catch (\Exception $e) {
            $last_version = false;
        }
        ...
        $admin_notifications = array(
                                ...
                                'last_version' => array(
                                                        ...
                                                        'build_day' => $last_version['build_time'],
                                                    ),
     ...
     * @return array
     */
    public function get_front_nav_list($update=false)
    {
        static $frontNav;
        if(!$frontNav || $update)
        {
            $frontNav = $this->fetch_all('nav',"status='Y'",'sort desc');
            $frontNav['installed'] = 1;
        }
        return $frontNav;
    }
}
