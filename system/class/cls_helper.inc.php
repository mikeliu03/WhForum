...
class H
{
	...
	public static function redirect_msg($message, $url = NULL, $interval = 5, $exit = true)
	{
		...
		if(is_mobile())
        {
            TPL::output('m/global/show_message');
        }else{
	...
	public static function display_options($param, $default = '_DEFAULT_', $default_key = 'key')
	{
		if (is_array($param))
		{
			...
			foreach ($param as $key => $value)
			{
				...
				{
					$output .= '<option value="' . $key . '"' . (($value == $default) ? '  selected' : '') . '>' . AWS_APP::lang()->_t($value) . '</option>';
				}
				else
				{
					$output .= '<option value="' . $key . '"' . (($key == $default) ? '  selected' : '') . '>' . AWS_APP::lang()->_t($value) . '</option>';
				}
			}
