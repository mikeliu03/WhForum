...
class topic_class extends AWS_MODEL
{
	...
	public function get_topic_list_by_js($where = null, $order = 'topic_id DESC', $limit = 100, $page = null,$isd=0,$uid=0)
	{
		...
		$topic_list=$isd>0?$topic_list:get_topic_btn($topic_list);
        return $topic_list;
     ...
     * @param $uid
     * @param int $limit
     * @param int $type 0获取列表，1 获取数量
     * @return bool|mixed
     * @throws \Zend_Exception
     */
	public function get_focus_topic_list($uid, $limit = 20,$type = 0)
	{
		if (!$uid)
		{
			return false;
		...
		if(!$type)
        {
            ...
            $topic_count = $this->count('topic', 'topic_id IN(' . implode(',', $topic_ids) . ')', 'discuss_count DESC');
            return $topic_count;
        }
	}
	...
	public function get_helpful_users_by_topic_ids($topic_ids, $limit = 10, $experience_limit = 1)
	{
		...
		{
			$rank_rate = $this->sum('reputation_topic', 'reputation', 'reputation > 0 AND topic_id IN(' . implode(',', array_unique($topic_ids)) . ') AND uid = ' . intval($best_answer_uid));
	...
	public function get_hot_topics($category_id = 0, $limit = 5, $section = null)
	{
		...
		if ($category_id)
		{
			if ($questions = $this->query_all("SELECT question_id FROM " . get_table('question') . " WHERE category_id IN(" . implode(',', array_unique($this->model('system')->get_category_with_child_ids('question', $category_id))) . ') ORDER BY add_time DESC LIMIT 200'))
			{
			...
			if (!$topic_relation = $this->fetch_all('topic_relation', 'item_id IN(' . implode(',', array_unique($question_ids)) . ") AND `type` = 'question'"))
			{
				return false;
			}
	...
	public function get_topic_best_answer_action_list($topic_ids, $uid, $limit)
	{
		...
		{
			if ($topic_relation = $this->query_all("SELECT item_id FROM " . $this->get_table('topic_relation') . " WHERE topic_id IN (" . implode(',', array_unique(explode(',', $topic_ids))) . ") AND `type` = 'question'"))
			{
				foreach ($topic_relation AS $key => $val)
				{
			...
			if ($best_answers = $this->query_all("SELECT question_id, best_answer FROM " . $this->get_table('question') . " WHERE best_answer > 0 AND question_id IN (" . implode(',', array_unique($question_ids)) . ") ORDER BY update_time DESC LIMIT " . $limit))
			{
				unset($question_ids);
