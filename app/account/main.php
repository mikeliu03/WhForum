...
class main extends AWS_CONTROLLER
{
	...
	{
	    run_hook('login_hook',['data'=>$_GET,'action'=>'before']);
		...
		if (is_mobile())
		{
			HTTP::redirect('/m/account/login/url-' . $_GET['url']);
		}
        ...
        run_hook('login_hook',['data'=>$_GET,'action'=>'after']);
	...
	{
        run_hook('register_hook',['data'=>$_GET,'action'=>'before']);
		...
		if(get_hook_info('mobile_regist')['state'] == 1){
            switch (get_plugins_config('mobile_regist')['register_valid_type'])
            {
                case 'mobile':
                    $type = 'mobile';
                    $other_valid = array();
        ...
        run_hook('register_hook',['data'=>$_GET,'action'=>'after']);
