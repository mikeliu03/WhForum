...
class ajax extends AWS_CONTROLLER
{
    ...
    public function first_find_password_action()
    {
       ...
       if (is_mobile())
       {
            ...
            $url = get_js_url('/m/account/find_password_success/mobile-'.$_POST['mobile'].'__type-mobile');
       }
        H::ajax_json_output(AWS_APP::RSM(array(
            'url' => $url
        ), 1, null));
	...
	{
		if ($log = $this->model('integral')->fetch_all('integral_log', 'uid = ' . $this->user_id, 'time DESC,integral ASC', calc_page_limit(intval($_GET['page']) ,10)))
		{
