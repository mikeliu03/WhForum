...
class core_uri
{
	...
	{
	    static $controllerDiy;
	    if(!$controllerDiy)
        {
            $controllerDiy = AWS_APP::model()->fetch_one('nav','url',"is_index=1 AND status='Y'");
        }
        $this->default_vars['app_dir'] = $controllerDiy ? $controllerDiy : $this->default_vars['app_dir'];
	...
	public function set_rewrite()
	{
        ...
        if(get_setting('mobile_base_url')!='' && get_setting('mobile_enable')=='Y')
        {
            if(base_parse_url()['sld'] == trim(get_setting('mobile_base_url')))
            {
