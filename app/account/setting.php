...
class setting extends AWS_CONTROLLER
{
	...
	public function profile_action()
	{
	    if(is_mobile())
        {
            HTTP::redirect('/m/people/profile/');
        }
	...
	public function privacy_action()
	{
        if(is_mobile())
        {
            HTTP::redirect('/m/people/privacy/');
        }
	...
	public function modify_mobile_action()
    {
        if(is_mobile())
        {
            if(!$_POST['new_mobile'])
            {
                H::ajax_json_output(AWS_APP::RSM(null, -1, '请输入手机号！'));
            }
            if($_POST['old_mobile']==$_POST['new_mobile'] && $_POST['old_mobile'])
            {
                H::ajax_json_output(AWS_APP::RSM(null, -1, '新手机和原手机不能相同'));
            }
            $mobile_new=trim($_POST['new_mobile']);
            $code_new=trim($_POST['new_code']);
            $mobile_old=trim($_POST['old_mobile']);
            $code_old=trim($_POST['old_code']);
            $url = get_js_url('/m/people/security/');

            if(!$this->model('tools')->checkSmsCode($mobile_new,$code_new))
            {
                H::ajax_json_output(AWS_APP::RSM($url, -1, '绑定手机号验证码有误或者已过期'));
            }

            if(!$this->model('tools')->checkSmsCode($mobile_old,$code_old))
            {
                H::ajax_json_output(AWS_APP::RSM($url, -1, '原手机号验证码有误或者已过期'));
            }
        }else{
            $mobile=trim($_POST['mobile']);
            $code=trim($_POST['code']);
            $url = '/';
            if(!$this->model('tools')->checkSmsCode($mobile,$code))
            {
                H::ajax_json_output(AWS_APP::RSM($url, -1, '验证码有误或者已过期'));
            }
        }

        if($this->model('account')->update('users',array('valid_mobile'=>1,'mobile'=>$mobile),'uid='.$this->user_id))
        {
            H::ajax_json_output(AWS_APP::RSM(['url'=>$url], 1, '绑定成功！'));
        }
	}
}
