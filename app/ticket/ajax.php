...
class ajax extends AWS_CONTROLLER
{
    ...
    public function publish_action()
    {
        ...
        if (intval(get_setting('report_message_uid')))
        {
            $this->model('notify')->send($this->user_id, intval(get_setting('report_message_uid')), notify_class::TYPE_TICKET_SUB, notify_class::CATEGORY_TICKET, 0, array(
    ...
    public function change_source_action()
    {
        if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_service'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('你没有权限更改工单')));
        ...
        if (!$_POST['source'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择工单来源')));
        }
        ...
        $this->model('ticket')->change_source($ticket_info['id'], $this->user_id, $_POST['source']);
