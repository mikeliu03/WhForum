...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
	    if(is_mobile())
        {
            HTTP::redirect('/m/publish/'.$_GET['id']);
        }
		if ($_GET['id'])
		{
        ...
        run_hook('publish_question_action_hook',['question_info'=>$question_info,'question_category_list'=>$question_category_list]);//发起问题页面渲染钩子
	...
	public function article_action()
	{
        if(is_mobile())
        {
            HTTP::redirect('/m/publish/article/'.$_GET['id']);
        }
		if ($_GET['id'])
		{
		...
		if (get_setting('category_enable') == 'Y')
		{
		    $article_category_list = $this->model('system')->build_category_html('question', 0, $article_info['category_id']);
			TPL::assign('article_category_list',$article_category_list);
		}
