...
class ACTION_LOG
{
    ...
    public static function delete_action_history($where,$whereDel=false)
    {
        if ($action_history = AWS_APP::model()->fetch_all('user_action_history', $where))
        {
            ...
            foreach ($action_history AS $key => $val)
            {
                AWS_APP::model()->update('user_action_history_data',array('is_del'=>$is_del),'history_id = ' . $val['history_id'].$delWhere);
                AWS_APP::model()->update('user_action_history_fresh',array('is_del'=>$is_del),'history_id = ' . $val['history_id'].$delWhere);
            }
            AWS_APP::model()->update('user_action_history',array('is_del'=>$is_del),$where);
        }
    }
	...
	public static function format_action_data_api($action, $uid = 0, $user_name = null, $associate_question_info = null, $associate_topic_info = null)
	{
		$user_link_attr = 'class="aw-user-name" data-id="' . $uid . '"';
		$user_profile_url = 'people/' . $uid;
		...
		switch ($action)
		{
			case self::ADD_QUESTION:
				if ($associate_topic_info)
				{
					$action_string = $associate_topic_info['topic_title']  . AWS_APP::lang()->_t('话题发起了一个问题');
				}
				else if ($associate_question_info['anonymous'])
				{
					$action_string = AWS_APP::lang()->_t('匿名用户') . ' ' . AWS_APP::lang()->_t('发起了问题');
				...
				{
					$action_string = $user_name . AWS_APP::lang()->_t('发起了问题');
				}
				break;

			case self::ADD_REQUESTION_FOCUS:
				$action_string = $user_name . AWS_APP::lang()->_t('关注了该问题');;
				break;

			case self::ANSWER_QUESTION:
				if ($associate_topic_info)
				{
					$action_string = $associate_topic_info['topic_title'] . AWS_APP::lang()->_t('话题新增了一个回答');
				}
				else
				{
					$action_string = $user_name . AWS_APP::lang()->_t('回答了问题');
				}
				break;
			...
			case self::ADD_AGREE: // '增加赞同'
				if ($associate_topic_info)
				{
					$action_string = $associate_topic_info['topic_title'] . AWS_APP::lang()->_t('话题添加了一个回复赞同');
				}
				else
				{
					$action_string =  $user_name . AWS_APP::lang()->_t('赞同了该回复');
				}
				break;

			case self::ADD_LIKE_PROJECT: // '增加赞同'
				$action_string = $user_name . AWS_APP::lang()->_t('喜欢了该活动');
				break;
			...
			case self::ADD_TOPIC : // '添加话题',
				if ($associate_topic_info)
				{
					if (isset($associate_topic_info[0]))
					{
						$action_string = $user_name . AWS_APP::lang()->_t('将该问题添加到');

						foreach ($associate_topic_info as $key => $val)
						{
							if ($val['topic_id'])
							{
								$action_string .= $val['topic_title'];
							}
							else
							{
								$action_string .= $val['topic_title'];
					...
					{
						$action_string = $user_name . AWS_APP::lang()->_t('将该问题添加到'). $associate_topic_info['topic_title'] . AWS_APP::lang()->_t('话题');
					}
				}
				else
				{
					$action_string = $user_name . AWS_APP::lang()->_t('添加了一个话题');
				}
				break;

			case self::ADD_TOPIC_FOCUS : // '关注话题',
				$action_string =  $user_name . AWS_APP::lang()->_t('关注话题');
				break;

			case self::ADD_ARTICLE :
				if ($associate_topic_info)
				{
					$action_string = $user_name . $associate_topic_info['topic_title'] . AWS_APP::lang()->_t('话题发表了文章');
				}
				else
				{
					$action_string = $user_name . AWS_APP::lang()->_t('发表了文章');
				}
				break;
			...
			case self::ADD_AGREE_ARTICLE :
				if ($associate_topic_info)
				{
					$action_string = $user_name . '在' . $associate_topic_info['topic_title'] . AWS_APP::lang()->_t('话题添加了一个文章赞同');
				}
				else
				{
					$action_string = $user_name . AWS_APP::lang()->_t('赞同了该文章');
				}
				break;
			...
			case self::ADD_COMMENT_ARTICLE :
				if ($associate_topic_info)
				{
					$action_string = $user_name . '在' . $associate_topic_info['topic_title'] . AWS_APP::lang()->_t('话题添加了一个文章评论');
				}
				else
				{
					$action_string = $user_name . AWS_APP::lang()->_t('评论了该文章');
				}
				break;
		}
		return $action_string;
