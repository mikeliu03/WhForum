...
class notify_class extends AWS_MODEL
{
    ...
    const TYPE_TICKET_SUB = 145;//提交工单通知
    ...
    const TYPE_SYSTEM = 183; // 系统消息
    const TYPE_USER_VERIFY = 190;
    ...
    public function send($sender_uid, $recipient_uid, $action_type, $model_type = 0, $source_id = 0, $data = array())
    {
        ...
        )))
        {
            ...
            if(get_hook_info('system_message')['state'] == 1)
            {
                ...
                $this->model('system_message')->update_notification_unread($recipient_uid);
     ...
     * * 获得通知列表
     * @param $recipient_uid
     * @param int $read_status 0 - 未读, 1 - 已读, other - 所有
     * @param null $limit
     * @return bool
     */
    public function list_notification($recipient_uid, $read_status = 0, $limit = null)
    {
        if(get_hook_info('system_message')['state'] == 1)
        {
            if (!$notify_ids = $this->model('system_message')->get_notification_list($recipient_uid, $read_status, $limit))
            {
                return false;
            }
        ...
        foreach ($notify_list as $key => $notify)
        {
            ...
            if ($data['from_uid'])
            {
                ...
                $tmp_data['p_url'] = ('/people/' . $user_info['uid']);
            }
            ...
            switch ($notify['model_type'])
            {
                ...
                case self::CATEGORY_TICKET:
                    ...
                    $tmp_data['ticket_title'] = $data['ticket_title'];
    ...
    public function list_notification_count($recipient_uid, $read_status = 0)
    {
        if(get_hook_info('system_message')['state'] == 1)
        {
            if (!$notify_ids = $this->model('system_message')->get_notification_list($recipient_uid, $read_status))
            {
                return false;
            }
        ...
        {
            if (!$notify_ids = $this->get_notification_list($recipient_uid, $read_status))
            {
                return false;
            }
        }
        ...
        foreach ($data AS $key => $val)
        {
            ...
            else
            {
                switch ($val['action_type'])
                {
                    ...
                    case self::TYPE_TICKET_SUB:
                        $data[$key]['message'] = AWS_APP::lang()->_t('您有新的 工单 (%s0) %s1', array(
                            $val['ticket_title'],
                            '<a href="' . get_js_url('/ticket/service-none__status-pending') . '">点击处理</a>'
                        ));
                        $val['title'] = $val['ticket_title'];
                        break;
                    ...
                    case self::TYPE_REPORT:
                        ...
                        break;
                    case self::TYPE_USER_VERIFY:
                        $data[$key]['message'] = '<a href="' . get_js_url('/admin/user/verify_approval_list/') . '">' . $val['title']. '</a>';
                    ...
                    case self::TYPE_SYSTEM:
                        $data[$key]['message'] = AWS_APP::lang()->_t('[系统消息]: ').
                            $data[$key]['title'];
                        break;
                }
            }
     ...
     */
    public function list_notification_ali($recipient_uid, $read_status = 0, $limit = null)
    {
        if(get_hook_info('system_message')['state'] == 1)
        {
            if (!$notify_ids = $this->model('system_message')->get_notification_list($recipient_uid, $read_status, $limit))
        ...
        foreach ($notify_list as $key => $notify)
        {
            ...
            switch ($notify['model_type'])
            {
                ...
                case self::CATEGORY_ARTICLE :
                    ...
                    $tmp_data['type'] = 'article';

                    $tmp_data['item_id'] = $data['article_id'];
                        ...
                        default :
                            ...
                            if ($notify['extends'])
                            {
                                ...
                                if ($answer_ids)
                                {
                                    ...
                                    $tmp_data['answer_ids'] = $answer_ids;
                            ...
                            else
                            {
                                ...
                                if ($data['item_id'])
                                {
                                    $tmp_data['answer_ids'][] = $data['item_id'];
                            ...
                            $tmp_data['type'] = 'question';

                            $tmp_data['item_id'] = $data['question_id'];
        ...
        return $this->format_notification_ali($list);
    }
     ...
     * @throws \Zend_Exception
     */
    public function format_notification_ali($data)
    {
        foreach ($data AS $key => $val)
        {
            if ($val['extend_count'] > 1)
            {
                ...
                {
                    case self::CATEGORY_QUESTION:
                        $data[$key]['message'] = $val['extend_count'] . ' ' . AWS_APP::lang()->_t('项关于问题') . $val['title'];

                        break;

                    case self::CATEGORY_ARTICLE:
                        $data[$key]['message'] = $val['extend_count'] . ' ' . AWS_APP::lang()->_t('项关于文章') . $val['title'];
            ...
            else
            {
                ...
                {
                    case self::TYPE_PEOPLE_FOCUS:
                        $data[$key]['message'] = $val['p_user_name'] . AWS_APP::lang()->_t('关注了你');

                        $val['title'] = $val['p_user_name'] . AWS_APP::lang()->_t('关注了你');
                    ...
                    case self::TYPE_NEW_ANSWER:
                        ...
                        {
                            $data[$key]['message'] = $val['p_user_name'];
                        }

                        $data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('回复了问题') . $val['title'];

                        break;

                    case self::TYPE_ARTICLE_NEW_COMMENT:
                        $data[$key]['message'] = $val['p_user_name'] . AWS_APP::lang()->_t('评论了文章') . $val['title'];
                    ...
                    case self::TYPE_COMMENT_AT_ME:
                        ...
                        $data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('在问题') . $val['title'] . AWS_APP::lang()->_t('中的评论提到了你');
                    ...
                    case self::TYPE_ARTICLE_COMMENT_AT_ME:
                        ...
                        $data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('在文章') . $val['title'] . AWS_APP::lang()->_t('评论中回复了你');
                    ...
                    case self::TYPE_ANSWER_AT_ME:
                        ...
                        $data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('在问题') . $val['title'] . AWS_APP::lang()->_t('中的回答提到了你');
                    ...
                    case self::TYPE_ANSWER_COMMENT_AT_ME:
                        ...
                        $data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('在问题') . $val['title']  . AWS_APP::lang()->_t('回答评论中提到了你');
                        break;

                    case self::TYPE_INVITE_QUESTION:
                        $data[$key]['message'] = $val['p_user_name'] . AWS_APP::lang()->_t('邀请你参与问题') . $val['title'];
                    ...
                    case self::TYPE_ANSWER_COMMENT:
                        ...
                        $data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('评论了你在问题') . $val['title'] . AWS_APP::lang()->_t('中的回复');
                    ...
                    case self::TYPE_QUESTION_COMMENT:
                        ...
                        $data[$key]['message'] .= ' ' . AWS_APP::lang()->_t('评论了你发起的问题') . $val['title'];

                        break;

                    case self::TYPE_ANSWER_AGREE:
                        $data[$key]['message'] = $val['p_user_name']. AWS_APP::lang()->_t('赞同了你在问题') . $val['title'] . AWS_APP::lang()->_t('中的回复');

                        break;

                    case self::TYPE_ANSWER_THANK:
                        $data[$key]['message'] = $val['p_user_name'] . AWS_APP::lang()->_t('感谢了你在问题') . $val['title'] . AWS_APP::lang()->_t('中的回复');

                        break;

                    case self::TYPE_MOD_QUESTION:
                        $data[$key]['message'] = $val['p_user_name'] . AWS_APP::lang()->_t('编辑了你发布的问题') . $val['title'];

                        break;

                    case self::TYPE_REMOVE_ANSWER:
                        $data[$key]['message'] = $val['p_user_name'] . AWS_APP::lang()->_t('删除了你在问题') . $val['title'] . AWS_APP::lang()->_t('中的回复');

                        break;

                    case self::TYPE_REMOVE_ANSWER_COMMENT:
                        $data[$key]['message'] =  $val['p_user_name'] . AWS_APP::lang()->_t('删除了你在问题') . $val['title'] . AWS_APP::lang()->_t('中的回复评论');

                        break;

                    case self::TYPE_REDIRECT_QUESTION:
                        $data[$key]['message'] = $val['p_user_name'] . AWS_APP::lang()->_t('重定向了你发起的问题'). $val['title'];

                        break;

                    case self::TYPE_QUESTION_THANK:
                        $data[$key]['message'] = $val['p_user_name']. AWS_APP::lang()->_t('感谢了你发起的问题') . $val['title'];

                        break;

                    case self::TYPE_CONTEXT:
                        $data[$key]['message'] = $val['title'];

                        break;

                    case self::TYPE_ARTICLE_APPROVED:
                        $data[$key]['message'] = AWS_APP::lang()->_t('你发起的文章 %s 审核通过', $val['title']);
                    ...
                    case self::TYPE_QUESTION_APPROVED:
                        $data[$key]['message'] = AWS_APP::lang()->_t('你发起的问题 %s 审核通过', $val['title']);
                    ...
                    case  self::TYPE_ARTICLE_VOTES:
                        ...
                        break;
                    case self::TYPE_REPORT:
                        $data[$key]['message'] = removeLinks($val['title']);
                    ...
                    case self::TYPE_VERIFY_SUCCESS:
                        ...
                        break;
                    case self::TYPE_VERIFY_FAIL:
                        $data[$key]['message'] = AWS_APP::lang()->_t('你发起的认证申请审核失败，原因').$val['title'];
                        break;
                    case self::TYPE_MEMBER_SUCCESS:
                        $data[$key]['message'] = AWS_APP::lang()->_t('你已成功开通会员，请前往 ').
                            '个人中心 查看会员到期时间';
