...
class ajax extends AWS_CONTROLLER
{
    ...
    {
        run_hook('upload_action_hook');//上传处理钩子若自定义处理结果，需在钩子中exit屏蔽执行下方逻辑
        if(get_hook_info('osd')['state']==1 and get_hook_config('osd')['group']['base']['config']['status']['value']!='no')
        {
            $ret = hook('osd','upload_files',['cat'=>$_GET['id'],'field'=>'aws_upload_file']);
    ...
    public function publish_question_action()
    {
        ...
        if ($this->publish_approval_valid(array(
            ...
            $question_id = $this->model('publish')->publish_question($_POST['question_content'], remove_xss($_POST['question_detail']), $_POST['category_id'], $this->user_id, $_POST['topics'], $_POST['anonymous'], $_POST['attach_access_key'], $_POST['ask_user_id'], $this->user_info['permission']['create_topic'],null);
            if ($_POST['_is_mobile'])
            {
    ...
    public function publish_article_action()
    {
        ...
        if (strlen($_POST['message']) > 65535) {
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('文章内容过长')));
        }
        ...
        if($_POST['logo_img'] AND $_POST['logo_img']!=G_STATIC_URL . '/css/default/img/default-cover.jpg')
        {
            $_POST['logo_img'] = '/uploads'.str_replace(get_setting('upload_url'),'',$_POST['logo_img']);
        }
        ...
        if ($this->publish_approval_valid(array(
            ...
            $article_id = $this->model('publish')->publish_article(trim($_POST['title']),$_POST['logo_img'], remove_xss($_POST['message']), $this->user_id, $_POST['topics'], $_POST['category_id'], $_POST['column_id'], $_POST['attach_access_key'], $this->user_info['permission']['create_topic']);
            if ($_POST['_is_mobile']) {
    ...
    public function modify_article_action()
    {
        ...
        if($_POST['logo_img'] AND $_POST['logo_img']!=G_STATIC_URL . '/css/default/img/default-cover.jpg')
        {
            if(strstr($_POST['logo_img'],'/uploads'))
            {
                $_POST['logo_img'] = str_replace('/uploads','',str_replace(get_setting('upload_url'),'',$_POST['logo_img']));
            }
            $_POST['logo_img'] = '/uploads'.$_POST['logo_img'];
        }
