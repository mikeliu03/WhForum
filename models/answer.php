...
class answer_class extends AWS_MODEL
{
		...
		if ($answer_info['uid'] != $uid)
		{
			...
			if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($answer_info['uid']))
			{
				...
				if ($weixin_user_info['weixin_settings']['NEW_COMMENT'] != 'N')
				{
                    ...
                    {
                        $this->model('weixin')->send_text_message($weixin_user['openid'], "您在 [" . $question_info['question_content'] . "] 中的回答收到了新评论:\n\n" . strip_tags(html_entity_decode($message)), $this->model('openid_weixin_weixin')->redirect_url('/m/question/' . $question_info['question_id'] . '?answer_id=' . $answer_info['answer_id'] . '&single=TRUE'));
                    }
                    if(get_setting('weixin_notify_type') == 'TEMPLATE')
                    {
                        ...
                        $this->model('weixin')->send_template_message($weixin_user['openid'],$template_info['template_id'],$this->model('openid_weixin_weixin')->redirect_url('/m/question/' . $question_info['question_id'] . '?answer_id=' . $answer_info['answer_id'] . '&single=TRUE'),$send_message);
                    }
				}
			}
		}
	...
	public function calc_best_answer()
	{
		...
		if ($questions = $this->query_all("SELECT question_id FROM " . $this->get_table('question') . " WHERE add_time < " . $start_time . " AND  best_answer = 0 AND answer_count > " . get_setting('best_answer_min_count')))
		{
			foreach ($questions AS $key => $val)
			{
				...
				{
					$this->set_best_answer($best_answer['answer_id'],$best_answer['uid'],2);
                    $this->model('question')->update('question',['best_answer'=>$best_answer['answer_id']],'question_id='.$best_answer['question_id']);
				}
			}
		}
		return true;
    ...
    public function remove_answer_by_id($answer_id,$whereDel=false)
    {
        if ($answer_info = $this->model('answer')->get_answer_by_id($answer_id))
        {
            ...
            $this->update_answer_comments_count($answer_id);
            $this->update('answer', ['is_del'=>$isdel],"answer_id = " . intval($answer_id).$delWhere);
            $this->model('question')->update('question',['best_answer'=>0],'question_id='.$answer_info['question_id']);
            $this->model('question')->update_answer_count($answer_info['question_id']);
            $this->model('question')->update_index_post_order($answer_info['question_id']);//更新首页排序
            ACTION_LOG::delete_action_history('associate_type = ' . ACTION_LOG::CATEGORY_ANSWER . ' AND associate_id = ' . intval($answer_id),$whereDel);
            ACTION_LOG::delete_action_history('associate_type = ' . ACTION_LOG::CATEGORY_QUESTION . ' AND associate_action = ' . ACTION_LOG::ANSWER_QUESTION . ' AND associate_attached = ' . intval($answer_id),$whereDel);
    ...
    //物理删除
    public function clear_answer($answer_id)
    {
    	if (!$answer_info = $this->get_answer_by_id(intval($answer_id)))
        {
            return false;
        }
    ...
    //物理删除
    public function clear_answer_comment($comment_id)
    {
        ...
        $this->delete('answer_comments',"id = " . intval($comment_id));

        $this->update_answer_comments_count($comment_info['answer_id']);
        return true;
    }
}