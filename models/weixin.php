...
class weixin_class extends AWS_MODEL
{
    ...
    /*发送模板消息通知*/
    public function send_template_message($openid,$template_id, $url,$message)
    {
        $app_id = get_setting('weixin_app_id');
        $app_secret = get_setting('weixin_app_secret');
        if (get_setting('weixin_account_role') != 'service' OR !$app_id OR !$app_secret)
        ...
        $result = $this->model('openid_weixin_weixin')->access_request($app_id, $app_secret, 'message/template/send', 'POST', $message_body);
        if (!$result)
        {
            return false;
        }
        if ($result['errcode'] == 40001)
        {
            return $this->send_template_message($openid, $message, $url);
        }
        return $result;
    }
