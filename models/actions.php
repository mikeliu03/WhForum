...
{   
	public function home_activity_api($uid, $limit = 10)
	{
		// 我关注的话题
		...
		foreach ($action_list as $key => $val)
		{
			...
			switch ($val['associate_action'])
			{
				...
				case ACTION_LOG::ADD_COMMENT_ARTICLE:
					...
					$article_user = $this->model('account')->get_user_info_by_uid($article_info['uid']);
					
					$article_info['user_name'] = $article_user['user_name'];

					$article_info['avatar_file'] = $article_user['avatar_file'];
				...
				default:
                    ...
                    $question_user = $this->model('account')->get_user_info_by_uid($question_info['published_uid']);

					$question_info['user_name'] = $question_user['user_name'];

					$question_info['avatar_file'] = $question_user['avatar_file'];
					
					$action_list[$key]['title'] = html_entity_decode($question_info['question_content']);
					...
					)))
					{
						...
						preg_match("/(href|src)=([\"|']?)([^\"'>]+.(jpg|JPG|jpeg|JPEG|gif|GIF|png|PNG))/i",strip_tags(FORMAT::parse_attachs(FORMAT::parse_bbcode($action_list[$key]['associate_content'])),'<img>'),$match);
                		$action_list[$key]['answer_info']['img']=$match[3];
                		$action_list[$key]['answer_info']['answer_content']=$action_list[$key]['associate_content'];
					}
					...
					preg_match("/(href|src)=([\"|']?)([^\"'>]+.(jpg|JPG|jpeg|JPEG|gif|GIF|png|PNG))/i",strip_tags(FORMAT::parse_attachs(FORMAT::parse_bbcode($question_info['question_detail'])),'<img>'),$match);
                	$action_list[$key]['question_info']['img']=$match[3];
			...
			if ($action_list[$key])
			{
				$action_list[$key]['last_action_str'] = ACTION_LOG::format_action_data_api($val['associate_action'], $val['uid'], $user_info_lists[$val['uid']]['user_name'], $question_info, $topic_info);
			}
		}
		...
		$action_list = array_values($action_list);
					...
					switch ($val['associate_action'])
					{
						...
						default:
							...
							)) AND $question_info['answer_count'])
							{
								...
								{
									$answer_list['answer_content'] = chinese_msubstr(html_entity_decode(FORMAT::parse_attachs(nl2br(FORMAT::parse_bbcode($answer_list['answer_content'])))),0,135,'utf-8');
