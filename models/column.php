...
class column_class extends AWS_MODEL
{
	...
	public function fetch_column_list($uid , $page, $limit = 10, $sort_type = 'sum' ,$type = false)
	{
		if($type){//获取本月
			...
			$order = 'recommend desc,add_time desc,sort ASC,focus_count desc';
			$where = 'is_verify = 1';
		}
	...
	{   
        ...
        $where[] = 'is_del=0';
        $article_list = $this->fetch_page('article', implode(' AND ', $where) , $order , $page, $limit);
        foreach ($article_list as $key=>$val)
        {
            $article_list[$key]['img_list'] = get_content_img($val['message'],false);
        }
        return $article_list;
	}
	...
	{   
		...
		if($category_ids)
		{
			return $this->sum('article','views',' is_del = 0 and column_id = '.$column_id.' and category_id not in ('.implode(",", $category_ids).')',1);
		}else
		{
			return $this->sum('article','views',' is_del = 0 and column_id = '.$column_id,1);
		}
    ...
    public function get_focus_column_list($uid, $limit = 20,$type=0)
    {
        if (!$uid)
        {
            return false;
        ...
        if($type){
            //获取总数
            $column_count = $this->count('column', 'column_id IN(' . implode(',', $column_ids) . ')', 'sort DESC');
            return $column_count;
        }else{
            //获取列表
    ...
    /*获取用户专栏列表*/
    public function fetch_user_column_list($uid , $page, $limit = 10, $sort_type = 'sum' ,$column_uid)
    {
        $order = 'recommend desc,add_time desc,sort ASC,focus_count desc';
        $where = $column_uid ? 'is_verify = 1 AND uid='.$column_uid : 'is_verify = 1';
        $list = $this->fetch_page('column', $where , $order , $page, $limit);
        if(!empty($list)){
            foreach ($list as $key => $val) {
                $list[$key] = $val;
