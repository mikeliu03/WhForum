...
class ticket_class extends AWS_MODEL
{
    ...
    public function reply_ticket($ticket_id, $message, $uid, $attach_access_key = null)
    {
        ...
        if (!$ticket_info['reply_time'] && $ticket_info['uid'] != $uid)
        {
    ...
    public function has_invited($ticket_id=0, $recipient_uid=0)
    {
    ...
    public function get_customer_list(){
        $users_groups = $this->fetch_all('users_group', "type=2 and custom=2");

        $result = array();
        foreach ($users_groups as $key => $va) {
            $result[$key]['group_name'] = $va['group_name'];

            $user_infos = $this->fetch_all('users', "is_del=0 and group_id=".$va['group_id']);

            foreach ($user_infos as $k => $v) {
                $v['avatar_file'] = get_avatar_url($v['uid'], 'mid');
                $v['url'] = get_js_url('/people/' . $v['uid']);
                $result[$key]['user_info'][$k] = $v;
            }
        }
    ...
    public function change_source($id, $uid, $source)
    {
        $ticket_info = $this->get_ticket_info_by_id($id);
        ...
        if (!in_array($source, array('local','weibo','weixin','email')))
        {
            return false;
        }
        ...
        $this->update('ticket', array('source' => $source), 'id = ' . $ticket_info['id']);

        $this->save_ticket_log($ticket_info['id'], $uid, 'change_source', array(
            'old' => $ticket_info['source'],
            'new' => $source
        ));
    ...
    public function get_work_list($where,$page = 1, $per_page = 10,$start_time,$end_time){
        $users_groups = $this->fetch_all('users_group', "type=2 and custom=2");
        foreach ($users_groups as $key => $va) {
            $group_ids[] += $va['group_id'];
        }

        $where = '';
        if($start_time){
            $where = ' and time >='.strtotime($start_time.' 00:00:00');
        }

        if($end_time){
            $where = ' and time <='.strtotime($end_time.' 23:59:59');
        }

        if($start_time && $end_time){
            $where = ' and time >='.strtotime($start_time.' 00:00:00').' and time<='.strtotime($end_time.' 23:59:59');
        }

        $result = array();

        if($group_ids){
            $user_list = $this->fetch_page('users', 'is_del=0 and group_id in ('.implode(',', array_unique($group_ids)).')', 'uid DESC', $page, $per_page);
            foreach ($user_list as $key => $va) {
                $result[$key]['uid'] = $va['uid'];
                $result[$key]['user_name'] = $va['user_name'];
                $result[$key]['no_deal_num'] = $this->count('ticket', "status='pending' and reply_time=0 and service=".$va['uid'].$where);
                $result[$key]['deal_num'] = $this->count('ticket', "status='pending' and reply_time>0 and service=".$va['uid'].$where);
                $result[$key]['closed_num'] = $this->count('ticket', "status='closed' and service=".$va['uid'].$where);
            }
        }
        return $result;
    }

    public function get_work_data_export($where,$start_time,$end_time){
        $users_groups = $this->fetch_all('users_group', "type=2 and custom=2");
        foreach ($users_groups as $key => $va) {
            $group_ids[] += $va['group_id'];
        }
        ...
        if($group_ids){
            $user_list = $this->fetch_all('users', 'is_del=0 and group_id in ('.implode(',', array_unique($group_ids)).')', 'uid DESC');
            foreach ($user_list as $key => $va) {
                $result[$key]['uid'] = $va['uid'];
                $result[$key]['user_name'] = $va['user_name'];
                $result[$key]['no_deal_num'] = $this->count('ticket', "status='pending' and reply_time=0 and service=".$va['uid'].$where);
    ...
    public function get_work_detail_list($uid,$page = 1, $per_page = 10,$start_time,$end_time){
        $where = '';
        if($start_time){
            $where = ' and time >='.strtotime($start_time.' 00:00:00');
        }
        ...
        return $this->fetch_page('ticket', "service=".intval($uid).$where, 'id DESC', $page, $per_page);
    }

    public function get_work_export($uid,$start_time,$end_time){
        $where = '';
        if($start_time){
            $where = ' and time >='.strtotime($start_time.' 00:00:00');
        }
        ...
        return $this->fetch_all('ticket', "service=".intval($uid).$where, 'id DESC');
    }
}
