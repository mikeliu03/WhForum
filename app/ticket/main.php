...
class main extends AWS_CONTROLLER
{
    ...
    public function index_action()
    {
        ...
        if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_service']
            AND $ticket_info['uid'] != $this->user_id AND !$this->model('ticket')->has_invited($ticket_info['id'],$this->user_id))
        {
        ...
        //客服列表
        TPL::assign('customer_infos', $this->model('ticket')->get_customer_list());
    ...
    public function channel_action()
    {
        if ($this->user_info['permission']['is_administortar'] OR $this->user_info['permission']['is_service'])
        {
            $this->crumb(AWS_APP::lang()->_t('渠道管理'), '/ticket/channel/');
            ...
            TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
                'base_url' => get_js_url('/ticket/channel/' . 'source-' . $_GET['source']),
                'total_rows' => $tickets_count,
                'per_page' => $this->pre_page
            ))->create_links());

            TPL::output('ticket/channel');
        }else
        {
            H::redirect_msg(AWS_APP::lang()->_t('你所在的用户组没有权限查看工单'));
        }
    ...
    public function work_data_action()
    {
        if ($_POST['action'] == 'search')
        {
            ...
            H::ajax_json_output(AWS_APP::RSM(array(
                'url' => get_js_url('/ticket/work_data/' . implode('__', $param))
            ), 1, null));
        }

        if ($this->user_info['permission']['is_administortar'] OR $this->user_info['permission']['is_service'])
        {
            $this->crumb(AWS_APP::lang()->_t('业绩报表'), '/ticket/work_data/');
            ...
            $tickets_list = $this->model('ticket')->get_work_list($filter, $_GET['page'], $this->per_page,$_GET['start_time'],$_GET['end_time']);
            ...
            TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
                'base_url' => get_js_url('/ticket/work_data/' . 'source-' . $_GET['source']),
                'total_rows' => $tickets_count,
                'per_page' => $this->pre_page
            ))->create_links());

            TPL::output('ticket/work_data');
        }else
        {
            H::redirect_msg(AWS_APP::lang()->_t('你所在的用户组没有权限查看工单'));
        }
    ...
    public function work_data_export_action()
    {
        if ($_POST['action'] == 'search')
        {
            foreach ($_POST as $key => $val)
            ...
            H::ajax_json_output(AWS_APP::RSM(array(
                'url' => get_js_url('/ticket/work_data_export/' . implode('__', $param))
            ), 1, null));
        }

        if ($this->user_info['permission']['is_administortar'] OR $this->user_info['permission']['is_service'])
        {
            $this->crumb(AWS_APP::lang()->_t('业绩报表导出'), '/ticket/work_data_export/');
            ...
            $tickets_list = $this->model('ticket')->get_work_data_export($filter,$_GET['start_time'],$_GET['end_time']);

            $this->model('excel')->data_export($tickets_list);
    ...
    public function work_detail_action()
    {
        if ($_POST['action'] == 'search')
        {
            foreach ($_POST as $key => $val)
            ...
            H::ajax_json_output(AWS_APP::RSM(array(
                'url' => get_js_url('/ticket/work_detail/' . implode('__', $param))
            ), 1, null));
        }

        if ($this->user_info['permission']['is_administortar'] OR $this->user_info['permission']['is_service'])
        {
            $this->crumb(AWS_APP::lang()->_t('业绩报表详情'), '/ticket/work_detail/');
            ...
            $users = $this->model('account')->get_user_info_by_uid(intval($_GET['uid']));
            TPL::assign('users', $users);

            $tickets_list = $this->model('ticket')->get_work_detail_list($_GET['uid'], $_GET['page'], $this->per_page,$_GET['start_time'],$_GET['end_time']);
            ...
            TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
                'base_url' => get_js_url('/ticket/work_dateil/uid-' . $_GET['uid'].'__start_time'.$_GET['start_time'].'__end_time'.$_GET['end_time']),
                'total_rows' => $tickets_count,
                'per_page' => $this->pre_page
            ))->create_links());

            TPL::output('ticket/work_detail');
        }else
        {
            H::redirect_msg(AWS_APP::lang()->_t('你所在的用户组没有权限查看工单'));
        }
    ...
    public function work_export_action()
    {
        if ($_POST['action'] == 'search')
        {
            foreach ($_POST as $key => $val)
            ...
            H::ajax_json_output(AWS_APP::RSM(array(
                'url' => get_js_url('/ticket/work_export/' . implode('__', $param))
            ), 1, null));
        }

        if ($this->user_info['permission']['is_administortar'] OR $this->user_info['permission']['is_service'])
        {
            $this->crumb(AWS_APP::lang()->_t('业绩报表详情导出'), '/ticket/work_export/');
            ...
            $tickets_list = $this->model('ticket')->get_work_export($_GET['uid'],$_GET['start_time'],$_GET['end_time']);
            ...
            $this->model('excel')->detail_exports($tickets_list);
        }else
        {
            H::redirect_msg(AWS_APP::lang()->_t('你所在的用户组没有权限查看工单'));
        }
