...
class project extends AWS_MOBILE_CONTROLLER
{
    ...
    public function index_action()
    {
        if(!is_mobile())
        {
            HTTP::redirect('/project/'.$_GET['id']);
        }

        if (!$project_info = $this->model('project')->get_project_info_by_id($_GET['id']))
        {
            H::redirect_msg(AWS_APP::lang()->_t('活动不存在或已被删除'));
        }
        ...
        $project_info['description'] = html_entity_decode(FORMAT::parse_attachs(nl2br(FORMAT::parse_markdown($project_info['description']))));
    ...
    public function index_square_action()
    {
        ...
        TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
            'base_url' => get_js_url('/project/category-'.$_GET['category'].'__sort-'.$_GET['sort']),
            'total_rows' => $this->model('project')->found_rows(),
            'per_page' => 9
        ))->create_links());
        TPL::output('m/project/square');
    }
    ...
    public function publish_action()
    {
        if(!is_mobile())
        {
            HTTP::redirect('/project/publish/'.$_GET['id']);
        }
        TPL::import_js('js/fileupload.js');
        if ($_GET['id'])
        {
        ...
        if (get_setting('category_enable') == 'Y')
        {
            ...
            TPL::assign('project_category_list', json_encode(array_values($article_category)));
        }
        TPL::output('m/project/publish');
    }
}