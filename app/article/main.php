...
class main extends AWS_CONTROLLER
{
	...
	{
        //文章前置钩子
        run_hook('article_detail_hook',['data'=>$_GET,'action'=>'before']);
		...
		$article_info['message'] = FORMAT::parse_imgs((FORMAT::parse_bbcode($article_info['message'])));
        hook('osd','replaceUrl',$article_info['message']);
		...
		//文章后置钩子
        run_hook('article_detail_hook',['data'=>$_GET,'action'=>'after']);
	...
	public function index_square_action()
	{
		...
		// 导航
		if (TPL::is_output('block/content_nav_menu'.TPL::$template_ext, 'article/square'))
		{
			TPL::assign('content_nav_menu', $this->model('menu')->get_nav_menu_list('article'));
		}

		//边栏热门话题
		if (TPL::is_output('block/sidebar_hot_topics'.TPL::$template_ext, 'article/square'))
		{
