...
class ajax extends AWS_CONTROLLER
{
	...
	public function upload_topic_pic_action()
	{
        ...
        if(get_hook_info('osd')['state']==1 and get_hook_config('osd')['group']['base']['config']['status']['value']!='no'){
                $ret=hook('osd','upload_files',['cat'=>'topic','field'=>'aws_upload_file']);
				$this->model('topic')->update_topic($this->user_id, $_GET['topic_id'], null, null, $ret['pic'].'?x-oss-process=image/resize,m_fixed,h_'.AWS_APP::config()->get('image')->topic_thumbnail['mid']['h'].',w_'.AWS_APP::config()->get('image')->topic_thumbnail['mid']['h'].'#');
                
                echo htmlspecialchars(json_encode(array(
                    'success' => true,
                    'thumb' => $ret['pic'].'?x-oss-process=image/resize,m_fixed,h_'.AWS_APP::config()->get('image')->topic_thumbnail['mid']['h'].',w_'.AWS_APP::config()->get('image')->topic_thumbnail['mid']['h'].'#'
                )), ENT_NOQUOTES);
            }else{
