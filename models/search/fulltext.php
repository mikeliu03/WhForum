...
class search_fulltext_class extends AWS_MODEL
{
	...
	public function bulid_query($table, $column, $q, $where = null)
	{
		...
		$search_type = array("question","article");
		if ($where && !in_array($table,$search_type))
		{
			$where = ' AND (' . $where . ')';
		}
	...
	public function search_questions($q, $topic_ids = null, $page = 1, $limit = 20, $is_recommend = false)
	{
		...
		if (is_array($q))
		{
			$search_key = implode(' ', $q);
		}
		$condition = $where.' OR question_detail like "%'.$search_key.'%"';

		$search_hash = $this->get_search_hash('question', 'question_content', $q, $condition);

		if (!$result = $this->fetch_cache($search_hash))
		{
			if ($result = $this->query_all($this->bulid_query('question', 'question_content', $q, $condition), $this->max_results))
			{
	...
	public function search_articles($q, $topic_ids = null, $page = 1, $limit = 20, $is_recommend = false)
	{
		...
		$condition = $where.' OR message like "%'.$search_key.'%"';

		$search_hash = $this->get_search_hash('article', 'title', $q, $condition);

		if (!$result = $this->fetch_cache($search_hash))
		{
			if ($result = $this->query_all($this->bulid_query('article', 'title', $q, $condition), $this->max_results))
			{
				$result = aasort($result, 'score', 'DESC');
			}
			else
