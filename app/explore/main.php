...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		...
		// 导航
		if (TPL::is_output('block/content_nav_menu'.TPL::$template_ext, 'explore/index'))
		{
			TPL::assign('content_nav_menu', $this->model('menu')->get_nav_menu_list('explore'));
		}

		// 边栏可能感兴趣的人
		if (TPL::is_output('block/sidebar_recommend_users_topics'.TPL::$template_ext, 'explore/index'))
		{
			TPL::assign('sidebar_recommend_users_topics', $this->model('module')->recommend_users_topics($this->user_id));
		}

		// 边栏热门用户
		if (TPL::is_output('block/sidebar_hot_users'.TPL::$template_ext, 'explore/index'))
		{
			TPL::assign('sidebar_hot_users', $this->model('module')->sidebar_hot_users($this->user_id, 5));
		}

		// 边栏热门话题
		if (TPL::is_output('block/sidebar_hot_topics'.TPL::$template_ext, 'explore/index'))
		{
			TPL::assign('sidebar_hot_topics', $this->model('module')->sidebar_hot_topics($category_info['id']));
		}

		// 边栏专题
		if (TPL::is_output('block/sidebar_feature'.TPL::$template_ext, 'explore/index'))
		{
		...
		if($_GET['is_reward'] && get_plugins_config('reward')['reward']['enable'] == 'Y')
		{
			$posts_list=$this->model('reward')->get_reward_question_list(intval($_GET['page']));
			...
			TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
				...
				'total_rows' => $this->model('reward')->found_rows(),
