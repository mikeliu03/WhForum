...
class main extends AWS_CONTROLLER
{
	...
	{
        //问题前置钩子
        run_hook('question_detail_hook',['data'=>$_GET,'action'=>'before']);
		...
		$question_info['question_detail'] = FORMAT::parse_imgs($question_info['question_detail']);
        ...
        //问题后置钩子
        run_hook('question_detail_hook',['data'=>$_GET,'action'=>'after']);
	...
	public function index_square_action()
	{
        ...
        if (is_mobile())
        {
            HTTP::redirect('/m/question/');
        }
		// 导航
		if (TPL::is_output('block/content_nav_menu'.TPL::$template_ext, 'question/square'))
		{
			TPL::assign('content_nav_menu', $this->model('menu')->get_nav_menu_list('question'));
		}

		//边栏可能感兴趣的人
		if (TPL::is_output('block/sidebar_recommend_users_topics'.TPL::$template_ext, 'question/square'))
		{
			TPL::assign('sidebar_recommend_users_topics', $this->model('module')->recommend_users_topics($this->user_id));
		}

		//边栏热门用户
		if (TPL::is_output('block/sidebar_hot_users'.TPL::$template_ext, 'question/square'))
		{
			TPL::assign('sidebar_hot_users', $this->model('module')->sidebar_hot_users($this->user_id, 5));
		}

		//边栏热门话题
		if (TPL::is_output('block/sidebar_hot_topics'.TPL::$template_ext, 'question/square'))
		{
			TPL::assign('sidebar_hot_topics', $this->model('module')->sidebar_hot_topics($_GET['category']));
		}

		//边栏专题
		if (TPL::is_output('block/sidebar_feature'.TPL::$template_ext, 'question/square'))
		{
