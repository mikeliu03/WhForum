...
{
	return base64_encode(base_url().'/' . str_replace('/' . G_INDEX_SCRIPT, '', substr($_SERVER['REQUEST_URI'], strlen(dirname($_SERVER['PHP_SELF'])))));
}
 ...
 * @return array
 */
function fetch_file_lists($dir, $file_type = null,$full_path = true)
{
	if ($file_type)
	{
	...
	while (($file = readdir($dir_handle)) !== false)
	{
		if (substr($file, 0, 1) != '.' AND !is_dir($base_dir . '/' . $file))
		{
			...
			{
				$files_list[] = $full_path ? $base_dir . '/' . $file : $file;
			}
		}
...
{
    if(get_setting('mobile_enable')=='Y')
    {
...
function base_parse_url()
{
    ...
    return array('scheme' =>http_type(),'sld'=>$server_sld,'domain'=>'.'.$server_domain,'request_one'=>$request[1]);
}
 ...
 * @return bool
 */
function mobile_redirect($open = true)
{
    if(get_setting('mobile_base_url')=='')
    {
        return false;
    }
    $parse_url = base_parse_url();
    $m_url = $parse_url['scheme'].trim(get_setting('mobile_base_url')).$parse_url['domain'];
    $web_url = $parse_url['scheme'].trim(get_setting('web_base_url')).$parse_url['domain'];
    $request_url = $_SERVER["REQUEST_URI"];
    if($open){
        if($parse_url['sld'] != get_setting('mobile_base_url'))
        {
...
/**
 * @param $url
 * @param bool $ignore_cookie
 * @return mixed
 */
function domain_replace($url,$ignore_cookie = false)
{
    if(get_setting('mobile_base_url')=='')
    {
        return $url;
    ...
    $parse_url = base_parse_url();
    $m_host = $parse_url['scheme'].get_setting('mobile_base_url');
    $web_host = $parse_url['scheme'].get_setting('web_base_url');
    if(is_mobile())
    {
        if (HTTP::get_cookie('_ignore_ua_check') == 'TRUE' && !$ignore_cookie)
        {
...
{
    $parse_url = base_parse_url();
    if(get_setting('web_base_url')=='')
    {
        return base_url();
    }
    return $parse_url['scheme'].get_setting('web_base_url').$parse_url['domain'].'/';
}
...
function get_mobile_url()
{
    ...
    $parse_url = base_parse_url();
    return  $parse_url['scheme'].get_setting('mobile_base_url').$parse_url['domain'].'/';
}

/**
 * 获取内容中图片列表
 * @param string $content 内容原始html
 * @param bool $replace 是否替换url为绝对路径
 * @return array 返回图片数组
 */
function get_content_img($content,$replace=true)
{
    ...
    if(!empty($out['src']))
    {
        foreach ($out['src'] as $k => $v)
        {
            if($replace)
            {
                $return[$k] = strpos($v,http_type())!== false ? $v : base_url().$v;
            }else{
                $return[$k] = $v;
            }
        }
 ...
 * @return string
 */
function action_log($action = null,$user_id = null, $record_id = null,$remark='')
{
    //参数检查
    ...
    $data['action']   = $action;
    $data['uid']     = $user_id;
    $data['action_ip']   = ip2long(fetch_ip());
    $data['item_id']   = intval($record_id);
    $data['remark'] = htmlspecialchars($remark);
    $data['add_time'] = time();
    return AWS_APP::model()->insert('action_log',$data);
}
 ...
 * @throws \Zend_Exception
 */
function thumb($file_path,$width,$height,$type='auto')
{
    if((strstr($file_path, 'http') || strstr($file_path, 'https')) && !strstr($file_path, base_url()))
    {
        $file_path = sync_img($file_path, 'thumb');
    }
    $file_path = str_replace('uploads/','',$file_path);
    $file_path = get_setting('upload_dir').$file_path;

    if(!file_exists($file_path))
    {
        return '/static/css/default/img/default-cover.jpg';
    }
    $file_info = explode('.',$file_path);
    $new_path = $file_info[0].'_w'.$width.'_h'.$height.'.'.$file_info[1];
    if(file_exists($new_path ))
    {
        return '/uploads'.str_replace(get_setting('upload_dir'),'',$new_path);
    }
    AWS_APP::picture()->resize($file_path,$new_path,$width,$height,$type);
    return '/uploads'.str_replace(get_setting('upload_dir'),'',$new_path);
}