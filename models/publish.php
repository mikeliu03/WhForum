...
class publish_class extends AWS_MODEL
{
	...
	public function publish_answer($question_id, $answer_content, $uid, $anonymous = null, $attach_access_key = null, $auto_focus = true, $reply_to_openid = true)
	{
		...
		if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($question_info['published_uid']) AND $question_info['published_uid'] != $uid)
		{
			...
			if ($weixin_user_info['weixin_settings']['NEW_ANSWER'] != 'N')
			{
                ...
                {
                    $this->model('weixin')->send_text_message($weixin_user['openid'], "您的问题 [" . $question_info['question_content'] . "] 收到了新的回答:\n\n" . strip_tags(html_entity_decode($answer_content)), $this->model('openid_weixin_weixin')->redirect_url('/m/question/' . $question_id));
                }
                if(get_setting('weixin_notify_type') == 'TEMPLATE')
                {
                    $template_info = $this->model('weixin')->fetch_row('weixin_template',"template_type = 'NEW_ANSWER'");
                    $send_message = parse_wx_template_msg($template_info['template_data'],$question_info['question_content'],strip_tags(html_entity_decode($question_info['question_detail'])),date('Y-m-d H:i',time()),strip_tags(html_entity_decode($answer_content)));
                    $this->model('weixin')->send_template_message($weixin_user['openid'],$template_info['template_id'],$this->model('openid_weixin_weixin')->redirect_url('/m/question/' . $question_id),$send_message);
                }
			}
		}
	...
	public function publish_question($question_content, $question_detail, $category_id, $uid, $topics = null, $anonymous = null, $attach_access_key = null, $ask_user_id = null, $create_topic = true, $from = null,$is_del = 0)
	{
		if ($question_id = $this->model('question')->save_question($question_content, $question_detail, $uid, $anonymous, null, $from,$is_del))
		{
			...
			if ($from AND is_array($from))
			{
				foreach ($from AS $type => $from_id)
				{
					...
					$this->update($type, array('question_id' => $question_id), 'id = ' . $from_id);
				}
			}
		}
		return $question_id;
	...
	public function publish_article($title,$logo_img, $message, $uid, $topics = null, $category_id = null,$column_id=null, $attach_access_key = null, $create_topic = true)
	{
        ...
        //百度推送，新增记录
        hook('bd_push','push',array('action'=>'urls','item_id'=>$article_id,'item_type'=>'article'));
		return $article_id;
	}
	...
	public function publish_article_comment($article_id, $message, $uid, $at_uid = null)
	{
		...
		if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($article_info['uid']) AND $article_info['uid'] != $uid)
		{
			...
			if ($weixin_user_info['weixin_settings']['NEW_ARTICLE_COMMENT'] != 'N')
			{
                if(get_setting('weixin_notify_type') == 'TEXT')
                {
                    $this->model('weixin')->send_text_message($weixin_user['openid'], "您的文章 [" . $article_info['title'] . "] 收到了新的评论:\n\n" . strip_tags(html_entity_decode($message)), $this->model('openid_weixin_weixin')->redirect_url('/article/' . $article_info['id']));
                }
                if(get_setting('weixin_notify_type') == 'TEMPLATE')
                {
                    $template_info = $this->model('weixin')->fetch_row('weixin_template',"template_type = 'NEW_ARTICLE_COMMENT'");
                    $send_message = parse_wx_template_msg($template_info['template_data'],$article_info['title'],strip_tags(html_entity_decode($article_info['message'])),date('Y-m-d H:i',time()),strip_tags(html_entity_decode($message)));
                    $this->model('weixin')->send_template_message($weixin_user['openid'],$template_info['template_id'],$this->model('openid_weixin_weixin')->redirect_url('/article/' . $article_info['id']),$send_message);
                }
			}
		}
	...
	public function parse_attach_data($attach, $item_type, $size = null)
	{
		...
		foreach ($attach as $key => $data)
		{
			...
			if ($data['is_image'] == 1 AND $size)
			{
				...
				$attach_list[$data['id']]['url_path'] = $attach_url . $data['file_location'];
			}
		}
