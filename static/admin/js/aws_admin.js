var AWS =
{
	...
	//官方账号 绑定
	checkBindStatus: function (selector)
	{
		var user_name = selector.parents('#aw-bind-box').find('.bind-name').val();
		var password = selector.parents('#aw-bind-box').find('.bind-pass').val();
		if(!user_name || !password)
		{
			return false;
		}
	    $.post('https://wenda.wecenter.com/cloud/ajax/bind_user/',{user_name:user_name,password:password},function(res)
	    {
	    	if(res.errno != 1)
	    	{
	    		$('#aw-bind-box').find('.msg').html(res.err).show();
	    		setTimeout(function(){
		            $('#aw-bind-box').find('.msg').hide();
		        },3000);
		        return false;
	    	}else{
				window.location.reload();
