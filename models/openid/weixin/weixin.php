...
class openid_weixin_weixin_class extends AWS_MODEL
{
    ...
    public function access_request($app_id, $app_secret, $url, $method, $contents = NULL)
    {
        ...
        $result = curl_request($url,$contents);
    ...
    public function upload_file($file, $type)
    {
        ...
        $result = curl_request(self::WEIXIN_API . 'media/upload?access_token=' . $this->get_access_token($app_id, $app_secret) . '&type=' . $type,  $post_data);
    ...
    public function get_weixin_user_by_uid($uid)
    {
        if (!is_digits($uid))
        {
            return false;
        ...
        static $weixin_user_info;

        if (!$weixin_user_info[$uid])
        {
            $weixin_user_info[$uid] = $this->fetch_row('users_weixin', 'uid = ' . $uid);
        }

        return $weixin_user_info[$uid];
    }
}
