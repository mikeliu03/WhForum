...
class ajax extends AWS_CONTROLLER
{
	...
	public function fetch_answer_data_action()
	{
		...
		{
            H::ajax_json_output($answer_info);
		}
	}
	...
	public function save_invite_action()
	{
		...
		if ($this->user_info['integral'] < 0 and get_setting('integral_system_enabled') == 'Y' and get_setting('integral_system_config_invite_answer')<0)
		{
	...
	public function agree_answer_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(array('action' => 'agree'), 1, null));
		}
		else
		{
			H::ajax_json_output(AWS_APP::RSM(array('action' => 'disagree'), 1, null));
		}
	}
	...
	{
        //问题回答评论钩子
        run_hook('save_question_answer_comment',['data'=>$_POST,'action'=>'before']);
		if (! $_GET['answer_id'])
		{
		...
		$comment_id = $this->model('answer')->insert_answer_comment($_GET['answer_id'], $this->user_id, $_POST['message']);
        //问题回答评论钩子
        run_hook('save_question_answer_comment',['data'=>$_POST,'action'=>'after','comment_id'=>$comment_id]);
	...
	public function save_question_comment_action()
	{
		...
		//问题评论钩子
        run_hook('save_question_comment',$_POST);
	...
	{
        run_hook('question_answer_vote_hook',['action'=>'before','data'=>$_POST]);
		$answer_info = $this->model('answer')->get_answer_by_id($_POST['answer_id']);
		...
		/*回复赞踩后置钩子*/
        run_hook('question_answer_vote_hook',['action'=>'after','data'=>$_POST]);
		H::ajax_json_output(AWS_APP::RSM(null, 1, null));
	}
	...
	{
		if ($this->user_info['integral'] < 0 AND get_setting('integral_system_enabled') == 'Y' and get_setting('integral_system_config_thanks')<0)
		{
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('你的剩余'.get_setting('integral_unit') .'已经不足以进行此操作')));
		}
	...
	public function question_answer_rate_action()
	{
		...
		if ($this->user_info['integral'] < 0 and get_setting('integral_system_enabled') == 'Y' and $_POST['type'] == 'thanks' and get_setting('integral_system_config_thanks')<0)
		{
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('你的剩余'.get_setting('integral_unit') .'已经不足以进行此操作')));
		}
	...
	{
        //问题回答前置钩子
        run_hook('save_question_answer_hook',['data'=>$_POST,'action'=>'before']);

		if ($this->user_info['integral'] < 0 and get_setting('integral_system_enabled') == 'Y' and get_setting('integral_system_config_new_answer')<0)
		{
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('你的剩余'.get_setting('integral_unit') .'已经不足以进行此操作')));
		}
        ...
        $_POST['answer_content'] = replace_image($_POST['answer_content'],'answer');

		$answer_content = remove_xss(trim($_POST['answer_content'], "\r\n\t "));
		...
		if (cjk_strlen(strip_tags(htmlspecialchars_decode($answer_content))) < get_setting('answer_length_lower'))
		{
		...
		else
		{
			...
			TPL::assign('answer_info', $answer_info);
            //问题回答后置钩子
            run_hook('save_question_answer_hook',['data'=>$_POST,'action'=>'after','answer_id'=>$answer_id]);
			if (is_mobile())
			{
				H::ajax_json_output(AWS_APP::RSM(array(
					'url' => get_js_url('/m/question/'.$answer_info['question_id'])
				), 1, null));
			}
			else
			{
	...
	public function update_answer_action()
	{
        ...
        //修改问题回答前置钩子
        run_hook('update_question_answer_hook',['data'=>$_POST,'action'=>'before','answer_id'=>$_GET['answer_id']]);
        ...
        //修改问题回答后置钩子
        run_hook('update_question_answer_hook',['data'=>$_POST,'action'=>'after','answer_id'=>$_GET['answer_id']]);

		if(is_mobile())
        {
            H::ajax_json_output(AWS_APP::RSM(array(
                'url' => get_js_url('/m/question/'.$answer_info['question_id']),
            ), 1, AWS_APP::lang()->_t('编辑回复成功')));
        }else{
            H::ajax_json_output(AWS_APP::RSM(array(
	...
	public function remove_question_action()
	{
		...
		if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator'] AND !($this->user_id == $question_info['published_uid'] && get_setting('user_remove_content_enable')=='Y'))
		{
		...
		$url = is_mobile() ? get_js_url('/m/') : get_js_url('/');
		H::ajax_json_output(AWS_APP::RSM(array(
			'url' => $url
		), 1, '删除成功'));
	}
	...
	public function set_recommend_action()
	{
        ...
        if(is_mobile())
        {
            H::ajax_json_output(AWS_APP::RSM(array('url'=>get_js_url('/m/question/'.intval($_POST['question_id']))), 1, AWS_APP::lang()->_t('操作成功')));
        }
		H::ajax_json_output(AWS_APP::RSM(null, 1, null));
	}
	...
	public function remove_comment_action()
	{
		...
		if(is_mobile())
        {
            H::ajax_json_output(AWS_APP::RSM(array('url'=>get_js_url('/m/question/answer_comment/answer_id-'.$comment['answer_id'])), 1, '删除成功'));
        }
	...
	public function answer_force_fold_action()
	{
		...
		$answer_info_integral=$this->model('account')->fetch_one('users','integral',"uid=".$answer_info['uid']);
		if ($answer_info_integral < 0 and get_setting('integral_system_enabled') == 'Y' and get_setting('integral_system_config_answer_fold')<0)
		{
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('你的剩余'.get_setting('integral_unit') .'已经不足以进行此操作')));
		}
	...
	public function one_best_answer_action()
	{
		...
		$_answer=$this->model('answer')->fetch_one('answer','answer_id',"question_id=".$answer_info['question_id']." and is_best=1");
		if($_answer)
		$this->model('answer')->update('answer',['is_best'=>0,'best_time'=>0,'best_uid'=>0],'answer_id='.$_answer);
		...
		if(is_mobile())
        {
            H::ajax_json_output(AWS_APP::RSM(array('url'=>get_js_url('/m/question/'.$question_info['question_id'])), 1, null));
        }else{
            H::ajax_json_output(AWS_APP::RSM(null, 1, null));
        }
	}
    ...
    public function set_top_question_action()
    {
        ...
        {
            case 'set':
                $this->model('posts')->set_top($_POST['question_id']);
                break;

            case 'unset':
                $this->model('posts')->unset_top($_POST['question_id']);
                break;
        }
        if(is_mobile())
        {
    ...
    public function remove_answer_action()
    {
        ...
        $this->model('question')->save_last_answer($answer_info['question_id']);
        H::ajax_json_output(AWS_APP::RSM(array('url'=>get_js_url('/m/question/'.$answer_info['question_id'])), 1, '删除成功'));
    }
}